    ...
    <article class="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-md shadow-slate-200/50 transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-lg">
        <div class="flex flex-col gap-6 lg:flex-row lg:justify-between">
            <div class="space-y-3 text-sm text-slate-600">
                ...
                <div class="flex flex-col gap-3 w-full sm:w-48 lg:w-52">
                    @if ($hasFiles)
                        <a href="{{ route('supervisor.submissions.show', $schedule) }}" class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 shadow-sm shadow-slate-200/70 transition-all duration-300 ease-in-out hover:border-indigo-200 hover:text-indigo-600 hover:shadow-md">
                            @include('layouts.partials.icon', ['name' => 'document', 'classes' => 'h-4 w-4 text-indigo-500'])
                            Berkas Guru
                        </a>
                    @else
                        <span class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-slate-200 bg-[#F9FAFB] px-4 py-2.5 text-xs font-medium text-slate-400 cursor-not-allowed">
                    ...
                    @if (!$schedule->conducted_at && $isDue)
                        <form action="{{ route('supervisor.schedules.conduct', $schedule) }}" method="post" onsubmit="return confirm('Tandai jadwal sudah dilaksanakan?')">
                            @csrf
                            <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-emerald-200/70 transition-all duration-300 ease-in-out hover:opacity-90 hover:shadow-lg hover:shadow-emerald-200/50">
                                @include('layouts.partials.icon', ['name' => 'check', 'classes' => 'h-4 w-4 text-white'])
                                Tandai Selesai
                            </button>
                        </form>
                    @elseif($schedule->conducted_at)
                        <span class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-xs font-semibold text-emerald-600 cursor-default">
                            @include('layouts.partials.icon', ['name' => 'check', 'classes' => 'h-4 w-4 text-emerald-500'])
                            Dilaksanakan
                        </span>
                    @endif

                    <a href="{{ route('supervisor.schedules.assessment', $schedule) }}" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-blue-500 px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-indigo-200/70 transition-all duration-300 ease-in-out hover:opacity-90 hover:shadow-lg hover:shadow-indigo-200/50">
                        @include('layouts.partials.icon', ['name' => 'sparkles', 'classes' => 'h-4 w-4 text-white'])
                        Penilaian
                    </a>
                    
                    <a href="{{ route('supervisor.schedules.edit', $schedule) }}" class="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm font-semibold text-amber-600 shadow-sm shadow-amber-100/60 transition-all duration-300 ease-in-out hover:border-amber-300 hover:bg-amber-100 hover:shadow-md">
...
@empty
    <div class="col-span-full">
        <x-empty-state 
            ...
            :message="request('q') || request('month') || request('year') ? 'Tidak ada jadwal yang cocok dengan filter yang dipilih.' : 'Belum ada jadwal sebagai pengawas.'"
            icon="calendar"
            actionLabel="Buat Jadwal Baru"
            :actionRoute="route('supervisor.schedules.create')"
        />
    </div>
@endforelse
