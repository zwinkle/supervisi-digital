      ...
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100 text-slate-600">
          @forelse ($invitations as $invitation)
            @php($schools = \App\Models\School::whereIn('id', (array) $invitation->school_ids)->pluck('name'))
            @php($link = \Illuminate\Support\Facades\URL::temporarySignedRoute('invites.accept.show', $invitation->expires_at ?? now()->addDays(7), ['token' => $invitation->token]))
